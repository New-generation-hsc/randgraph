...
class update_strategy_t<randomwalk_conf_t, empty_data_t, FirstOrder, SampleType>
{
public:
    static void update_walk(const randomwalk_conf_t &conf, const walker_t<empty_data_t> &walker, graph_cache *cache, graph_walk<empty_data_t, FirstOrder> *walk_manager, SampleType *sampler)
    {
        ...
        while (dst >= start_vert && dst < end_vert && hop > 0)
        {
            ...
            {
                walk_context<UNBAISEDCONTEXT> ctx(dst, walk_manager->nvertices, run_block->csr + adj_head, run_block->csr + adj_tail, &seed, conf.teleport);
                dst = vertex_sample(ctx, sampler);
            }
            else
            {
                walk_context<BIASEDCONTEXT> ctx(dst, walk_manager->nvertices, run_block->csr + adj_head, run_block->csr + adj_tail, &seed, run_block->weights + adj_head, run_block->weights + adj_tail, conf.teleport);
                dst = vertex_sample(ctx, sampler);
            }
            hop--;
        }
